...
class Player(pygame.sprite.Sprite):
   ...
   def update_bar(self, surface, screen_size, speed_timer):
       pygame.draw.rect(surface, (60, 63, 60), [screen_size[0]/2 - self.max_health, screen_size[1]/2-23, self.max_health*2, 6])
       pygame.draw.rect(surface, (111, 210, 46), [screen_size[0]/2 - self.max_health, screen_size[1]/2-23, self.health*2, 6])
       pygame.draw.rect(surface, (60,23,60), [5, 5, screen_size[0]-10, 5])
       pygame.draw.rect(surface, (30,144,255), [5, 5, self.xp*(50/(math.exp((self.level)/5))), 5])
       if self.speed >= 6:
            pygame.draw.rect(surface, (0, 0, 255), [5,12, (5-(time.time()-speed_timer))*(screen_size[0])/5-10, 2])
   ...
   def add_xp(self, amount, screen_size):
       self.xp += amount
       if self.xp*(50/(math.exp((self.level)/5))) >= screen_size[0]:
           self.xp = (self.xp*(50/(math.exp((self.level)/5))) - 1070) /(50/(math.exp((self.level)/5)))
           self.level +=1
           self.damage += 1
           self.max_health += 2
           self.health += 2
